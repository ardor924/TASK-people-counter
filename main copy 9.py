...
# =========================
# #2 VLM Attribute Engine (Custom Storage Path)
# =========================
class VLMAttributeEngine:
    def __init__(self):
        self.available = False
        # 모델 저장 경로를 프로젝트 내 models/ 폴더로 지정
        ...
        try:
            ...
            print("[INFO] CLIP library missing. Installing...")
            subprocess.check_call([sys.executable, "-m", "pip", "install", "git+https://github.com/openai/CLIP.git"])
            import clip

        # 2. 모델 로드 (download_root를 통해 경로 임의 지정)
        try:
            import torch
            self.device = "cuda" if torch.cuda.is_available() else "cpu"
            
            # download_root 파라미터를 사용하면 원하는 폴더에 모델을 받습니다.
            print(f"[INFO] Loading CLIP model to {self.model_dir}...")
            ...
            self.color_queries = ["black clothing", "white clothing", "blue clothing", "red clothing", "gray clothing"]
            ...
            self.available = True
            print(f"[INFO] VLM Engine Ready on {self.device}")
        except Exception as e:
            print(f"[WARN] VLM Load Failed: {e}")
    ...
    def analyze(self, crop_cv2):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Softmax를 통한 확률 비교 (편향 방지를 위해 직접 비교)
...
def main():
    ...
    while True:
        ...
        cv2.putText(display, f"MODEL: models/ViT-B-32.pt", (20, 130), 1, 0.9, (150, 150, 150), 1)
