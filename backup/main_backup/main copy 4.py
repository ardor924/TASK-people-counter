...
RESIZE_WIDTH = 960
CONF_THRESH = 0.65      # 야간 오탐지 방지를 위해 임계값 살짝 상향 (0.62 -> 0.65)
DWELL_TIME_SEC = 0.5    
FORGET_TIME_SEC = 0.7   
...
def main():
    ...
    while True:
        ...
        # 자동차(가로가 긴 형태)를 걸러내기 위해 세로/가로 비율이 1.5 미만인 객체는 제거
        # 야간 전조등 번짐 대응을 위해 더 엄격하게 적용 (1.2 -> 1.5)
        valid_indices = []
                ...
                if tid not in id_attributes:
                    ...
                    if track_hits[raw_id] == 41:
                        ...
                        if candidates:
                            ...
                            # [방어 로직] 2. 확신도 필터링 강화
                            # 야간 노이즈로 인한 오판 방지를 위해 임계값 조정 (40.0 -> 50.0)
        ...
        cv2.putText(display, f"FPS  : {frame_idx/(time.time()-start_time):.1f} | Night Defense ON", (20, 135), 1, 0.9, (150, 150, 150), 1)
    ...
    report_content = f"""
===================================================================================================================
[FINAL SYSTEM REPORT] {os.path.basename(VIDEO_PATH)}
===================================================================================================================
 1. GENDER DISTRIBUTION SUMMARY
    - Total Tracked: {next_display_id-1} | Male: {gender_stats['Male']} / Female: {gender_stats['Female']} / Unk: {gender_stats['Unknown']}
-------------------------------------------------------------------------------------------------------------------
 2. PERFORMANCE & QUALITY
    - Effective FPS: {frame_idx/elapsed:.1f} (4070Ti Optimized)
    - Counting Accuracy: {min(total_count/25, 25/total_count)*100:.1f}%
    - Defense Logic: Aspect Ratio(>1.5) + Confidence(>50%) Filtering
===================================================================================================================
"""
    print(report_content)
    
    # 리포트 파일 저장
    report_filename = f"logs/report_{os.path.basename(VIDEO_PATH)}.txt"
    try:
