...
def main():
    ...
    while True:
        ...
        # [NEW] 오탐지 방지: 기하학적 필터링 (Aspect Ratio)
        # 야간 영상에서 가로로 넓은 자동차나 빛 번짐을 제거하기 위해 h/w 비율이 낮은 객체 필터링
        valid_indices = []
        for i, xyxy in enumerate(detections.xyxy):
            ...
            # 사람은 일반적으로 세로가 가로보다 훨씬 깁니다 (h/w > 1.25 권장)
            # confidence threshold와 함께 비율 체크 적용
            if aspect_ratio > 1.25 and detections.confidence[i] >= CONF_THRESH:
                ...
                if tid not in id_attributes:
                    ...
                    if track_hits[raw_id] == 41:
                        ...
                        if candidates:
                            ...
                            # 비정상 데이터 방어 (확신도 45% 미만은 Unknown - 야간 환경 보정)
                            if g_conf < 45.0: final_g = "Unknown"
        ...
        cv2.putText(display, f"VLM BEST-FRAME (NIGHT): {total_count}", (20, 45), 1, 1.8, (255, 255, 255), 2)
        cv2.putText(display, f"MALE : {gender_stats['Male']} | FEMALE : {gender_stats['Female']}", (20, 95), 1, 1.2, (0, 255, 255), 1)
        cv2.putText(display, f"FPS  : {frame_idx/(time.time()-start_time):.1f} | Night Optimized", (20, 135), 1, 0.9, (150, 150, 150), 1)
    ...
    print("\n" + "=" * 115)
    print(f"[FINAL REPORT] NIGHT-OPTIMIZED VLM ATTRIBUTE ANALYSIS")
    print("=" * 115)
    ...
    print(f"    - Anti-False Positive Strategy: Aspect Ratio Filtering Enabled")
    print("=" * 115)
